...
<div class="slider">
            ...
            <div class="slider-wrapper">
               <div class="slider-main">
                  <div class="slider-item">
                     <img src="<c:url value="/assets/image/img/${ product.id_img }"></c:url> " alt="" />
                  </div>
                  <div class="slider-item">
                     <img src="<c:url value="/assets/image/img/${ product.id_img }"></c:url>" alt="" />
                  </div>
	...
	<div class="contents">
            <form class="content-information" method="get" action="<c:url value="/Booking/${ product.id_product }"></c:url>">
               <div class="in-heading">
                  <p class="home-sleep">Phòng ngủ: ${ product.bedroom }</p>
                  <p class="home-shower">Phòng tắm: ${ product.barthroom }</p>
                  <p class="home-area">Diện tích: ${ product.area }</p>
                  <p class="home-price">${ product.price }</p>
                  <p onclick="Toggle1()" id='btnh1' class="btn"><i class="fa fa-heart" aria-hidden="true" ></i> </p>
               </div>
               <div class="content-about">
                  <h1 class="about-heading">${ product.name }</h1>
                  <p class="home-adrees">${ product.address }</p>
                  ...
                  <div class="btn-contact">
						...
						<c:choose>
							<c:when test="${product.status == 1 }">
								<button class="status">Đang Bán</button>
								<c:if test="${ not empty LoginInfo }">
			<button type="submit" class="contact">Đặt Lịch</button>
		</c:if>
							</c:when>
							<c:when test="${product.status == 2 }">
								<button class="statuss">Đang Tư Vấn</button>
								<c:if test="${ not empty LoginInfo }">
			<button type="submit" class="contact">Đặt Lịch</button>
		</c:if>
							</c:when>
							<c:otherwise>
            					<button class="statusss">Đã Bán</button>
            					<c:if test="${ not empty LoginInfo }">
			<button style="display:none;" type="submit" class="contact">Đặt Lịch</button>
		</c:if>
        					 </c:otherwise>
						</c:choose>
